...
function updateGuestCounters() {
    const ageInputs = document.querySelectorAll('.age-input');
    const ages = Array.from(ageInputs).map(function(input) {
    ...
    if (minors > 0) {
        const minorAges = ages.filter(function(age) {
            return age < 18 && age > 0;
        });
...
document.addEventListener('input', function(e) {
    if (e.target.id === 'occupants') {
        const occupants = Number.parseInt(e.target.value, 10) || 0;
...
document.getElementById('bookingForm').addEventListener('submit', function(e) {
    ...
    const occupants = Number.parseInt(document.getElementById('occupants').value, 10);
...
function displayResults(data, unitName, arrival, departure, inputAges) {
    ...
    if (responseData['Location ID'] || responseData['Total Charge']) {
        ...
        const adultCount = inputAges.filter(function(age) {
            return age >= 18;
        }).length;
        
        const childCount = inputAges.filter(function(age) {
            return age < 18;
        }).length;
        
        const childAges = inputAges.filter(function(age) {
            return age < 18;
        });
        ...
        html += '<div class="result-item">' +
            ...
            (childCount > 0 ? ', ' + childCount + ' Child' + (childCount !== 1 ? 'ren' : '') : '') + '</span></div>';
